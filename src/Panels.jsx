...
export function Rules() {
  return (
    <div className="rules-content">
      ...
      <h3>병종과 합병</h3>
      <p>
        전투는 문명 6 방식이에요. 전투력(CS)은 창병 21, 기병 25, 머스킷병 28(사격)
        / 22(근접 방어), 포병 34(포격) / 10(근접 방어). 피해 = 30 × e^((공격 전투력 −
        방어 전투력) ÷ 25) × 무작위 75~125%. 전투력이 같으면 평균 30, 10 높으면 약
        45를 주고 20을 받아요. 근접 전투는 양쪽이 동시에 같은 공식으로 피해를 주고,
        사격·포격은 공격자만 피해를 줘요. 보정은 모두 전투력에 더해져요: 창병은
        기병 상대 +10, 기병은 창병 상대 +5, 포병은 유닛 상대 −17, 방어 태세 +6(준비
        중 +3), 구릉지 방어 +3, 요새·주둔지 +4, 강 건너 공격 −5, 도하 직후 −3, 목표에
        인접한 아군 부대마다 측면 +2, 방어자에 인접한 아군 부대마다 지원 +2, 부상은
        체력 0에서 −10까지 비례, 베테랑 레벨마다 +1.
      </p>
      <p>
        창병은 자원 없이 생산하고, 기병은 말 2, 머스킷병은 초석 2, 포병은 철 2·초석 2가
        필요해요. 한 유닛은 대대예요. 인접한 같은 병종은 같은 편제끼리만 합쳐요:
        대대+대대→여단(전투력 +10), 여단+여단→사단(전투력 +17). 대대는 여단에 합칠 수
        없고 사단은 더 합칠 수 없어요. 합병해도 체력은 100으로 유지되며(더 건강한
        쪽을 이어받음) 경험치는 편성 수로 가중 평균해요.
      </p>
      <h3>도시와 성벽</h3>
      <p>
        도시 전투력은 주둔 부대의 방어 전투력과 도시 자체 전투력(기본 20 + 인구당 3 +
        성벽 레벨당 3) 중 큰 값이에요. 성벽 체력은 레벨당 50(50/100/150). 공격은 성벽을
        먼저 때리고, 성벽이 서 있는 동안 근접 공격은 15%, 사격은 50%, 포격은 100%의
        피해만 성벽에 줘요. 성벽이 무너지면 도시 본체가 피해를 받고 사격은 50%만
        들어가요. 성벽이 있는 도시는 턴마다 도시 전투력으로 2칸 안의 적을 포격하고,
        인접 근접 공격자에게 같은 공식으로 반격해요. 도시 본체는 공격이 멈춘 뒤
        턴당 20 회복하고, 성벽은 공격받지 않는 동안 생산력으로 수리해요. 본체가 0이
        되면 인접한 근접 부대만 점령할 수 있어요.
      </p>
      <h3>전투 결과와 경험치</h3>
      <p>
        전투에서 두 부대가 모두 전멸하는 일은 없어요. 서로 치명타를 주고받으면
        남은 전력이 더 높은 쪽(동률이면 공격자)이 체력 1로 살아남아요. 전투에
        참여한 부대는 매 전투마다 경험치 +1, 적 부대를 격파하면 +3, 상대 전투력이
        10 이상 높거나 역상성이거나 상대 편제가 더 클 때 격파하면 +5를 얻어요.
      </p>
      <h3>이동과 지형</h3>
      <p>
